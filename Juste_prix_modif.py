...
try:
		...
		try:
			...
			while True:
				if Prix_Jouer > Juste_Prix:
					...
					print(f"Vous avez gagné, le Juste prix etais: {Juste_Prix}")
					exit()
		...
		try:
			...
			while True:
				if i == 1:
					if Prix_Jouer == Juste_Prix:
						i3 = 10 - i 
						print(f"Vous avez gagné. En total de {i3} essaie, et le juste prix etait: {Juste_Prix}")
						exit()
					else:
						print(f"Vous avez perdue, le Juste prix etais: {Juste_Prix}")
						exit()		
		
				elif Prix_Jouer > Juste_Prix:
					i-= 1
					print(f"Il vous reste {i} essaie")
					Prix_Jouer = int(input("Plus petit: "))
	...
	# ----------------  Niveau 3 ------------------
		...
		try:
			...
			print(f"Il faut choisir un nombre entre 1 et {Prix_Max}")
			...
			if Nombre_essai != 0:
				if i2 == 1:
					if Prix_Jouer != Juste_Prix2:								
						print(f"Vous avez perdue, le Juste prix etais: {Juste_Prix2}")
						exit()
					else:
						i2 - 10
						print(f"Vous avez gagné. En total de {i2} essaie, et le juste prix etait: {Juste_Prix2}")
						exit()
		
				elif Prix_Jouer > Juste_Prix2:
					i2-= 1
					print(f"Il vous reste {i2} essaie")
					Prix_Jouer = int(input("Plus petit: "))
					...
					i3 = Nombre_essai - i2
					print(f"Vous avez gagné. En total de {i3} essaie, et le juste prix etait: {Juste_Prix2}")
					exit()



		
			# ---------- nombre d'essais ilimité --------------			
				...
				if Prix_Jouer > Juste_Prix2:
					...
					print(f"Vous avez gagné, le Juste prix etais: {Juste_Prix2}")
					exit()
